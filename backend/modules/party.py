...
def load_static_data():
    ...
    print(
        ...
        f"{len(DB_ITEMS)} oggetti, {len(DB_MOVES)} mosse, {len(DB_ABILITIES)} abilita, {len(DB_SPECIES)} specie, "
        f"{len(DB_SPECIES_BASE_STATS)} base stats, {len(DB_SPECIES_IDENTITY_META)} identity meta, "
        f"{len(DB_SPECIES_GROWTH_RATES)} growth rates, {len(DB_SPECIES_ABILITIES_META)} species abilities meta."
    )
