...
def _read_flag_bit(buf, section_id, rel_offset, bit_index):
    ...
    if abs_offset < 0 or abs_offset >= len(buf):
        return False
    return ((buf[abs_offset] >> bit_index) & 1) == 1
...
def read_event_flag(buf, flag_id):
    flag = int(flag_id)
    if EXPANDED_FLAGS_BASE <= flag < 0x1900:
...
def count_badges(buf):
    return sum(1 for flag_id in range(FLAG_BADGE01_GET, FLAG_BADGE08_GET + 1) if read_event_flag(buf, flag_id))
...
def _has_item(slots, item_id):
    return _sum_item_quantity(slots, item_id) > 0


def _read_money(buf):
    section = _find_active_section_by_id(buf, money_mod.TRAINER_SECTION_ID)
    if not section:
        return 0
    return int(money_mod.ru32(section["data"], money_mod.FALLBACK_OFF_MONEY))


def _read_bp(buf):
    section = _find_active_section_by_id(buf, 4)
    if not section:
        return 0
    return int(money_mod.ru16(buf, int(section["off"]) + 0xF34))


def build_game_progress_snapshot(buf, cap_profile=CAP_PROFILES["NORMAL"]):
    ...
    return {
        ...
        "mega_unlocked": any(_has_item(owned_slots, item_id) for item_id in MEGA_ACCESSORY_ITEM_IDS),
        ...
        "key_items": {
            ...
            "stat_scanner": _has_item(owned_slots, ITEM_STAT_SCANNER),
            "mega_ring": _has_item(owned_slots, ITEM_MEGA_RING),
        },
        "consumables": {
