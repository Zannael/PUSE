...
bool MutatePartyMon(
    ...
    const uint32_t team_count = std::min(ReadU32Le(trainer, kTeamCountOff), static_cast<uint32_t>(6));
...
void RefreshPartyMonChecksums(std::vector<uint8_t> &buffer) {
    ...
    for (const auto &sec : sections) {
        ...
        uint8_t *trainer = &buffer[sec.offset];
        const uint32_t team_count = std::min(static_cast<uint32_t>(6), ReadU32Le(trainer, kTeamCountOff));
        for (uint32_t i = 0; i < team_count; ++i) {
